...
async def create_template(
    db: AsyncSession, template_input: TemplateCreate
) -> Templates:
...
async def get_template(db: AsyncSession, template_id: int) -> Templates | None:
    result = await db.execute(
        select(Templates).where(
            Templates.id == template_id, Templates.deleted_at.is_(None)
        )
    )
...
async def get_all_templates(db: AsyncSession) -> list[Templates]:
    templates = await db.execute(
        select(Templates).where(Templates.deleted_at.is_(None))
    )
